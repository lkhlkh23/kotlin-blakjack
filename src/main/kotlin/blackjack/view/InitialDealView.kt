...
fun initialize(members: List<Member>, deck: Deck) {
    announce(members)
...
private fun announce(
    ...
    val playerNames: String = format(members.filter { !it.isDealer() }
        .map {it.name}
        .toList())
...
private fun dealCardsForAll(members: List<Member>, deck: Deck) {
    for (member in members) {
        dealCardsForSingle(member, deck)
    }
}

private fun dealCardsForSingle(member: Member, deck: Deck) {
